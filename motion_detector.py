...
class_to_label = {0: 'Angry', 1: 'Disgust', 2: 'Fear',
                  3: 'Happy', 4: 'Sad', 5: 'Surprise', 6: 'Neutral'}

face_cascade = cv2.CascadeClassifier(
    "facial_expression/haarcascade_frontalface_default.xml")
first_frame = None
...
def frames(frame1, frame2, text, status_list):
    ...
    if len(faces) == 0:
        for (x, y, w, h) in faces:
            cv2.putText(frame2, "Processing", (x, y-10),
                        cv2.FONT_HERSHEY_COMPLEX, 1, (255, 30, 0), 2, cv2.LINE_AA)
    ...
    for face in faces:
        ...
        if (np.all(np.array(face_section.shape))):
            ...
            cv2.putText(frame2, label1, (x, y-10),
                        cv2.FONT_HERSHEY_COMPLEX, 1, (125, 30, 120), 2, cv2.LINE_AA)
    ...
    for (box, pred) in zip(locs, preds):
        ...
        label2 = "Covered" if mask > withoutMask else "Uncovered"
        color = (0, 255, 0) if label2 == "Covered" else (0, 0, 255)
...
while video.isOpened():
    ...
    try:
        if mode == "night":
            frame_y, frame_x, blur, dilated, frame, text, label1, label2 = frames(
                frame1, frame2, text, status_list)
            frame_x = imutils.resize(frame_y, width=800)
            frame = imutils.resize(frame_x, width=800)
            dilated = imutils.resize(dilated, width=800)
            frame = imutils.resize(frame, width=800)
            blur = imutils.resize(blur, width=800)
            cv2.imshow("Feed", frame_x)
            cv2.imshow("Color", frame_x)
            cv2.imshow("Guassian Blur", blur)
            cv2.imshow("Dilate", dilated)
            cv2.imshow("Covered/Uncovered", frame)

        elif mode == 'day':
            _, _, _, _, frame, text, label1, label2 = frames(
                frame1, frame2, text, status_list)
            frame = imutils.resize(frame, width=800)
            cv2.imshow("Covered/Uncovered", frame)
...
# Taking the image of the frame-----------------------------------------------------------------------
    ...
    if (text == "Occupied" and currentframe % 200 == 0):
        ...
        if len(emotions) == 0:
            ...
            # images.caption_this_image(name) +
